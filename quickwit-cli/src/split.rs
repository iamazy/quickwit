...
pub fn build_split_command<'a>() -> Command<'a> {
    Command::new("split")
        .about("and manage splits.")
        .subcommand(
            Command::new("list")
                .about("Lists the splits of an index.")
                .args(&[
                    arg!(--config <CONFIG> "Config file location").env("QW_CONFIG").required(true),
                    arg!(--index <INDEX> "Target index ID").required(true),
                    ...
                    arg!(--"end-date" <END_DATE> "Selects the split that contain documents before this date (time-series indexes only)."),
                    arg!(--tags <TAGS> "Selects the splits whose tags are ")
                        .use_value_delimiter(true),
